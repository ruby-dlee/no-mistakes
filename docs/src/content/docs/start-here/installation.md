...
If exact, unexpired review, repair, or test worker leases exist, the update refuses to restart the daemon and prints each worker's job, run, role, head, and lease owner. Historical pending/running rows and durable CI waits do not block the update. Pass `--force` to proceed despite a real lease; `-y`/`--yes` does **not** bypass this guard.
