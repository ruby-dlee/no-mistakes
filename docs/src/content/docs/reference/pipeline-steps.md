...
- With the default `session_reuse: true`, Claude, Codex, Grok, Pi, and Antigravity may reuse the first fixer session, but a second semantic repair starts fresh; a same-file recurrence after that fresh repair parks for primary-agent handoff, a resume failure retries the same turn fresh, and unsupported agents run cold
...
**Approval:** required if any finding has severity `error` or `warning`. Findings with `action: ask-user` pause for approval instead of entering the normal auto-fix loop. Review deterministically rewrites every high-risk finding and every contract/schema, parser/serialization, auth/permission, security/safety, deploy/routing, generated-artifact, product-behavior, or integration/compatibility finding to `ask-user`, even when the reviewer mis-scores the overall risk low or medium; an owner-selected fix then enters the structured semantic-repair path. With the default `auto_fix.review: 0`, other blocking review findings also park even when their action is `auto-fix`; setting repo or global `auto_fix.review` above `0` re-enables the automatic review fix loop only for eligible local-mechanical findings. Findings with `action: no-op` are informational only. The shared [finding-action model](/no-mistakes/concepts/auto-fix/#finding-actions) owns the behavior for a missing `action`.
...
The fixer applies all selected fixes before running one bounded verification set limited to the changed area. Its structured proof names a public/executable regression that failed before and passes after, the relevant integration or consumer compatibility result, and the disposition of any generated artifact. Generated output may change only with its authoritative source and an available emitter that was actually run; otherwise it remains untouched and the repair hands off. The fixer is instructed not to run the complete repository test or lint suite during the fix round.
